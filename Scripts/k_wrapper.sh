	...
	1) echo "ROI - Independent Components Analysis"
	  echo "--------------------"
		...
		if [[ $mode == "subj" ]]; then
			...
			qsub -N ICA_$cond -e $output/$cond/logs/$DATE/ica_$name.err -o $output/$cond/logs/$DATE/ica_$name.out -hold_jid 'roi_*' $script_path/Dependencies/ICA_ROI.sh $subj $cond $output $script_path $nROI $mode
