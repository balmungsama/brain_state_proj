	  ...
	  echo "--------------------"
		...
		if [[ $mode == "subj" ]]; then
			...
			for subj in $(ls $path); do
				...
				qsub -N roi_$name -e $script_path/logs/$DATE/roi_$name.err -o $script_path/logs/$DATE/roi_$name.out $script_path/Dependencies/rm_vols.sh $path/$subj $cond $output $script_path $nROI $mode
