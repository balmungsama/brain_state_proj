	...
	1) echo "ROI - Independent Components Analysis"
	  echo "--------------------"
		...
		if [[ $mode == "subj" ]]; then
			...
			qsub -N ICA_$cond -e $script_path/logs/$DATE/ica_$cond.err -o $script_path/logs/$DATE/ica_$name.out -hold_jid /roi_ $script_path/Dependencies/ICA_ROI.sh $path/$subj $cond $output $script_path $nROI $mode
