...
for( k_val in kk ){
	for( k_rep in kk_reps ){
		...
		optim_tmp_row <- data.frame(k_val = kk, BIC = AIC.BIC$BIC, AIC = AIC.BIC$AIC)
...
for(k_val in kk) {
	ksumm_BIC_tmp_col <- c( mean(k_optim_tab$BIC[which(k_optim_tab$k_val == kk)]), 
						      sd(k_optim_tab$BIC[which(k_optim_tab$k_val == kk)]) )
	ksumm_AIC_tmp_col <- c( mean(k_optim_tab$AIC[which(k_optim_tab$k_val == kk)]), 
						      sd(k_optim_tab$AIC[which(k_optim_tab$k_val == kk)]) )

	if( !exists('ksumm_BIC') ) {
		ksumm_BIC <- data.frame( summ = ksumm_BIC_tmp_col,  rownames = c('Mean', 'SD') )
		ksumm_AIC <- data.frame( summ = ksumm_AIC_tmp_col,  rownames = c('Mean', 'SD') )

		colnames(ksumm_BIC) <- print0('k', kk)
		colnames(ksumm_AIC) <- print0('k', kk)

	} else {
		ksumm_BIC[, print0('k', kk)] <- ksumm_BIC_tmp_col
		ksumm_AIC[, print0('k', kk)] <- ksumm_AIC_tmp_col
	}
}
...
kplot <- ggplot() + 
			geom_line(mapping = aes(x = kk, y = ksumm_BIC['Mean',], colour = 'AIC'), size = 1) +
			geom_line(mapping = aes(x = kk, y = ksumm_AIC['Mean',], colour = 'BIC'), size = 1) +

			geom_errorbar(mapping = aes(x = kk, ymax = ksumm_AIC['Mean',] + (ksumm_AIC['SD',]/sqrt(kk_reps)), ymin = ksumm_AIC['Mean',] - (ksumm_AIC['SD',]/sqrt(kk_reps) * z_conf_lvl), colour = 'AIC' ), size = 1) +
			geom_errorbar(mapping = aes(x = kk, ymax = ksumm_BIC['Mean',] + (ksumm_BIC['SD',]/sqrt(kk_reps)), ymin = ksumm_BIC['Mean',] - (ksumm_BIC['SD',]/sqrt(kk_reps) * z_conf_lvl), colour = 'BIC' ), size = 1) + 

			geom_vline(mapping = aes(xintercept = kk[which(ksumm_BIC['Mean',] == min(ksumm_BIC['Mean',]))], colour = 'BIC'), size = 1, linetype = 'dashed' ) +
			geom_vline(mapping = aes(xintercept = kk[which(ksumm_AIC['Mean',] == min(ksumm_AIC['Mean',]))], colour = 'AIC'), size = 1, linetype = 'dashed' ) +
