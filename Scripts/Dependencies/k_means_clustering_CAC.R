  ...
  stop
	...
	for(subj in subj_dirs) {
		...
		for(mat in mat_files) {
			...
			if(lateralized == T) {    # if rois are lateralized, the matrix is organized by Left & Right hemispheres
				...
				# print(dim(tmp_mat_subj))
				if(dim(tmp_mat_subj) != 93) {next}
	...
	for( k_val in kk ){
		...
		for( k_rep in 1:kk_reps ){
			...
			write.table(mat_stacked, file = '/home/hpc3586/stacked_mat.txt')

			mat_stacked[which(mat_stacked == Inf, arr.ind = T)] <- 0
	...
	for(clus_k in 1:kk) {
		print(clus_k)
		k_ls_names   <- row.names(cluster_ls)[which(cluster_ls == clus_k)]
		# print(k_ls_names)
		k_summ_mat   <- matrix(0, ncol = length(labels), nrow = length(labels) )
		k_summ_count <- 0
		for(win_nm in k_ls_names){
			# if( is.na(as.numeric(get(win_nm))[4,7]) == T ) {
			# 	print('an NA is here')
			# 	next
			# }

			tmp_mat <- as.matrix( get(win_nm) )
			k_summ_mat   <- k_summ_mat + tmp_mat
			k_summ_count <- k_summ_count + 1
		}

		k_summ_mat <- k_summ_mat / k_summ_count
		colnames(k_summ_mat)  <- NULL
		row.names(k_summ_mat) <- NULL
		k_summ_mat <- fisherz(k_summ_mat)

		write.csv( x = k_summ_mat, file = file.path(TOP_DIR, '..', 'k_clusters', paste0('cluster_summ_', clus_k, '_rep', k_rep, '.csv')) )
	}
